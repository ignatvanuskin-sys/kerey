(async () => {
  ...
  const closeDlg = async () => { const d = document.querySelector('[role="dialog"]'); if (!d) return true; const b = [...d.querySelectorAll('button')].find(x => x.getAttribute('aria-label') === 'Закрыть'); if (b) b.click(); await sleep(300); return !dlgOpen(); };
  ...
  headerBtn.click(); await sleep(700);
  const d1 = document.querySelector('[role="dialog"]');
  out.headerClick = { opened: !!d1, step: d1 ? d1.innerText.replace(/\n+/g, ' | ').slice(0, 60) : null, closed: await closeDlg() };

  heroBtn.click(); await sleep(700);
  const d2 = document.querySelector('[role="dialog"]');
  out.heroClick = { opened: !!d2, step: d2 ? d2.innerText.replace(/\n+/g, ' | ').slice(0, 60) : null, closed: await closeDlg() };

  cardBtn.scrollIntoView({ block: 'center' }); await sleep(250);
  cardBtn.click(); await sleep(700);
  const d3 = document.querySelector('[role="dialog"]');
  out.cardClick = { opened: !!d3, step: d3 ? d3.innerText.replace(/\n+/g, ' | ').slice(0, 60) : null, closed: await closeDlg() };

  // tel: link
  const contacts = document.getElementById('contacts');
  const cPhone = contacts.querySelector('a[href^="tel:"]');
  cPhone.scrollIntoView({ block: 'center' }); await sleep(250);
  const before = location.href;
  cPhone.click(); await sleep(600);
  out.phoneClick = { href: cPhone.getAttribute('href'), urlBefore: before, urlAfter: location.href, urlChanged: location.href !== before };
  return out;
})()
